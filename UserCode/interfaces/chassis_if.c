...
void Chassis_Init(Chassis_t* chassis, const Chassis_Config_t* config)
{
    ...
    memcpy(&chassis->feedback, &config->feedback_source, sizeof(chassis->feedback));
...
static void update_chassis_velocity_control(Chassis_t* chassis)
{
    ...
    { // 如果基于世界坐标计算速度，则需要转为车身坐标系，并应用到底盘驱动器
      ...
      //                                    &chassis->world_velocity,
      //                                    &chassis->body_velocity);
      // // 临时写一下来调试
      // const float        beta          = DEG2RAD(0.5f * chassis->body_velocity.wz * 1e-3f);
      // const float        cot_beta      = 1.0f / tanf(beta);
      // Chassis_Velocity_t temp_velocity = {
      //     .vx = beta * (chassis->body_velocity.vx * cot_beta + chassis->body_velocity.vy),
      //     .vy = beta * (chassis->body_velocity.vy * cot_beta - chassis->body_velocity.vx),
      //     .wz = chassis->body_velocity.wz
      // };
      ...
      //                             temp_velocity.wz);
        ...
        // ChassisDriver_ApplyVelocity(&chassis->driver,
        //                             chassis->body_velocity.vx,
        //                             chassis->body_velocity.vy,
        //                             chassis->body_velocity.wz);
    }
    else
    {
        // 否则基于机体坐标计算速度，则不需要变动
...
void update_chassis_position_control(Chassis_t* chassis)
{
    ...
    Chassis_Velocity_t body_velocity = {
        .vx = relative_posture.x * 1.0f,
        .vy = relative_posture.y * 1.0f,
        .wz = relative_posture.yaw * 1.0f,
    };
                                    ...
                                    const Chassis_PostureTarget_t* relative_target)
{
    ...
    Chassis_BodyPosture2WorldPosture(chassis, &relative_target->posture, &absolute_target);
    chassis->posture.target.posture.x   = absolute_target.x;
    chassis->posture.target.posture.y   = absolute_target.y;
    chassis->posture.target.posture.yaw = absolute_target.yaw;
    chassis->posture.target.speed       = relative_target->speed;
    chassis->posture.target.omega       = relative_target->omega;
 ...
 *                        没做出来，暂时去掉本参数
 */
void Chassis_SetVelWorldFrame(Chassis_t* chassis, const Chassis_Velocity_t* world_velocity)
{
    // chassis->target_in_world   = target_in_world;
 ...
 *                        没做出来，暂时去掉本参数
 */
void Chassis_SetVelBodyFrame(Chassis_t* chassis, const Chassis_Velocity_t* body_velocity)
{
    // chassis->target_in_world  = target_in_world;
    chassis->velocity.in_body.vx = body_velocity->vx;
    chassis->velocity.in_body.vy = body_velocity->vy;
